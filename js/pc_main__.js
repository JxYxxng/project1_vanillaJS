window.addEventListener("load",function(){
	...
	for(var i=0; i<depth1.length; i++){
		...
		var depth2Ul=depth1[i].children[1];
		var depth2Li=depth2Ul.children;
	...
	// 뒤에 붙일 경우 : appendChild(node)
	// 앞에 붙일 경우 : insertBefore(node)
	...
	next.addEventListener("click", function(e){
		...
		n4=n4+1;
		distance2=167*n4;
		var currentx=Math.abs(logoUl.offsetLeft);
		// console.log(currentx+" : "+distance2); // 0 : 167

		var movingTimer=setInterval(function(){
			if(Math.abs(distance2-currentx) > 2){
				currentx+=5;
